...
import uuid
import logging
from scapy.all import ARP, Ether, send, srp, RadioTap, Dot11, sendp, Dot11Deauth
import random
import subprocess
import signal
...
class orb1t:
    def __init__(self):
        ...
        self.log_file = "wifi_jammer.log"  # Log file for outputs
        self.setup_logging()
        self.running = True
        self.packet_count = 0
    ...
    def port_scan(self):  # Ensure method is defined correctly
...
# Example of how to call the function
    ...
    def validate_interface(self, interface):
        # Check if the interface exists and can be used for monitor mode
...
""")
    ...
    def main_menu(self):
        while True:
            self.print_header()  # Print header and warning
    ...
    def generate_icmp_packet(self):
        # Basic implementation of packet generation
    ...
    def send_packets(self, target_ip):
        ...
        while self.running:
            try:
                ...
                self.report_packets()  # Optionally report progress during the attack

            except Exception as e:
                self.log(Fore.RED + f"TCP/UDP Error: {e}")

        # After the attack finishes, report how many packets were sent
        end_time = time.time()
        total_duration = end_time - start_time
        self.log(Fore.GREEN + f"Attack completed. Sent {self.packet_count} packets in {total_duration:.2f} seconds.")
    
    def stop_attack(self):
        # Call this function to stop the attack
        self.running = False
    ...
    def get_mac(self, ip_address):
        # This function retrieves the MAC address for a given IP address
        try:
    ...
    def send_arp_spoof(self, target_ip, gateway_ip, target_mac, gateway_mac):
        try:
            while self.running:
                # Send ARP spoofing packets to target and gateway
...
def send_http_requests(self, target_ip, payload):
    # Load User-Agent strings from the file at the start
     ...
     while self.running:
        try:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                ...
                # Randomize User-Agent; use a default if the list is empty
...
if __name__ == "__main__":
    tool = orb1t()
    tool.main_menu()
    orb1t_instance = orb1t()  # Create an instance of orb1t
